...
不配置 AI，也可以布置小屋、出门旅行、收集内置明信片。想收到新画的照片，或者亲手做一个自己喜欢的伙伴，可以继续往下试试。

**角色形象、整套装扮、生活场景、家具、随身物品、互动动作和明信片，都支持自定义。下载源码后，还可以修改按钮、图标、界面配色等可见内容，把《旅行栗栗》做成自己喜欢的样子。AI 可以帮你制作这些素材，也可以协助修改对应的配置与代码。**

### 让旅行来信自由生成

在「设置 → 开发者设置」中，为文字和图片接口分别填写 **协议、Base URL、模型名、API Key**，然后保存。在下一次出发时，文字模型会结合目的地、角色描述、装扮和行囊写下旅行内容，图片模型根据来信和参考形象绘制沿途照片。

同时配置文字和图片 API，就能生成新的图文来信；只配置图片 API，也可以配合本地文案生成照片。仅配置文字 API 时，当前仍使用内置图库。

**配置可用的 Key 后，不设每日三次生成限制。** 每程按设置的来信节点逐张生成，后续旅程可以继续生成；供应商的费用、余额和速率限制仍然适用。保存配置本身不会发起模型测试，生成请求失败时会回退到本地文案和默认图片。

### 用 AI 制作自己的伙伴和小屋

想换掉的内容，可以分别制作，也可以整理成一套完整素材包。

- **角色和装扮**可以换成自己设计的小伙伴，为它准备不同的整套衣服，也可以导入你已有的形象素材。
- **场景和家具**可以重新绘制小屋背景，添一张喜欢的椅子、一盏灯，或做一整套风格统一的家具。
- **随身物品和互动动作**可以自己设计。想让新伙伴坐下来读书、喝茶，需要一起准备它与家具互动的画面。
- **明信片和界面外观**也能修改。可以更换默认旅行图库；按钮图案、图标、面板和配色则通过源码中的界面资源与样式调整。

具体可以这样做。

1. 下载本仓库，把 [素材制作指南](docs/AI-ASSET-AUTHORING.md)、[素材包规范](docs/ASSET-PACK-SPEC.md) 和你想要的形象、画风交给支持生图的 AI 工具；需要协助配置时，再交给能编辑代码的 AI 工具。
2. 生成独立的角色图、背景、空家具和对应互动帧，按规范整理图片与配置。角色图和互动帧需要分别准备，换一张头像不会自动重画所有动作。
3. 在「设置 → 开发者设置 → 素材管理」中导入素材，或导入带有 `pack.json` 的完整素材包。修改按钮和界面样式时，可参考 [界面皮肤说明](docs/UI-SKIN.md)，在本地版本中调整。

**页面内配置的 API 目前用于旅行来信生成。角色、场景、家具等外观自定义，需要先制作素材，再导入使用；制作好的素材可以在未配置 Key 时继续使用。**
...
Base URL、模型名称、尺寸和质量参数请按供应商的说明填写。页面输入的 Key 仅保存在当前页面内存，刷新后需重新填写，不会写入存档或素材包。

...
| 模式 | 来信文字 | 旅行照片 |
| --- | --- | --- |
| **默认图库模式：不配置 API** | 使用本地旅行文案 | 按目的地选用内置旅行插画，覆盖全部 228 个目的地 |
| **AI 生成模式：同时配置可用的文字和图片 API** | 根据目的地、角色装扮、随身物品生成新来信 | 根据同一段旅程与所选照片风格生成对应的新照片 |

**228 张内置旅行插画已接入**，覆盖中国 34 个省级地区与其他 194 个国家，每个目的地一张。图库使用栗栗的自在原色、暖冬围巾、雨天来信三套形象，搭配 36 种游戏物品表现绘画、摄影、野餐、演奏、散步等 38 类活动。图片按需加载，游玩时不消耗 API。

内置照片中的穿搭、道具和动作已预先绘制，不会随玩家当前装备变化；同一目的地的多封来信会使用同一张内置照片。旧相册里的通用占位图会补为目的地图片，已生成、导入的私人照片和正在进行的图片请求会保留。完整图片目录、场景来源与提示词见 [`TRAVEL-PHOTOS.json`](dist/packs/reading-room/TRAVEL-PHOTOS.json)。

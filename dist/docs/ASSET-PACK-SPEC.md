...
最多 512 个图片资源；每类最多 100 个定义；每场景最多 100 个家具实例；每家具最多 50 个动作；每个组合最多 100 帧。单张不超过 30MB，边长不超过 16384；ZIP 最多 750 个文件、解压后合计 512MB。实际导入会检查路径、尺寸、引用、基础回退和重复 ID。
...
上面两个目标素材需要另行在 `assets` 声明并提供文件。当前示例的映射留空，尚未生成各地专属默认图，暂用现有示例插画。未来可覆盖全部国家与34个中国省级地区，ZIP 总大小仍受512MB限制，制作时应控制图片体积。

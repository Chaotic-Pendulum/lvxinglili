...
export function photoStylePickerView(app) {
  ...
  return `<section class="photo-style-panel" id="photo-style-panel"><h2>旅行照片风格</h2><p class="field-help">从下一次旅行起用于生成新照片，当前旅程与内置照片保持原样。</p><details class="photo-style-picker"><summary><span data-selected-photo-style>${photoStyleSelectionView(app)}</span><span class="photo-style-expand">选择</span></summary><button type="button" class="btn small photo-style-default" data-photo-style="${PACK_STYLE_ID}" aria-pressed="${selected.id === PACK_STYLE_ID}">${icon("leaf", 16)} 素材包默认</button><div class="photo-style-grid">${PHOTO_STYLES.map(
    (style, index) => {
